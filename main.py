...
def log_and_run():
    ...
    if items is None:
        error_message = start_message + "\n[錯誤] 爬蟲抓取失敗，可能被 591/CDN 擋下或頁面載入失敗，請查看 log。\n" + end_message
        push_to_line(error_message)
        print("[Scheduler] Crawler finished with error.")
        return

    if not items:
        push_to_line(start_message + "\n[資訊] 本次沒有找到符合條件的租屋物件。\n" + end_message)
        print("[Scheduler] No items found; LINE message sent.")
    ...
    if not message:
        ...
        print("[Scheduler] No new items after filtering pushed IDs; LINE message sent.")
        print("[Scheduler] Crawler finished.")
        return
